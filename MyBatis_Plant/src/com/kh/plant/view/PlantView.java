...
public class PlantView {
	...
	public void mainMenu() {
		...
		while(true) {
			...
			System.out.println("4. 식물 성장 상태 변경 ");
			System.out.println("5. 식물 삭제 ");
			System.out.println("6. 식물 상세 조회 ");
			System.out.println("7. 식물 이름 키워드로 검색 ");
	...
	private void updateStatus() {
		...
		System.out.print("식물 식별번호를 입력해주세요 > ");
		int plantId = sc.nextInt();
		sc.nextLine();
		System.out.print("성장상태 변경을 할 식물 이름을 입력해주세요 > ");
		String plantName = sc.nextLine();
		System.out.print("식물 성장상태를 입력해주세요 >");
		String plantStatus = sc.nextLine();
		
		int result = pc.updateStatus(plantId, plantName, plantStatus);
		
		if(result > 0) {
			System.out.println("식물 성장상태 변경을 완료하였습니다. ");
		} else {
	...
	private void deletePlant() {
		...
		System.out.println("========= 식물 정보 삭제 ===========");
		System.out.print("정보를 삭제할 식물 식별번호를 입력해주세요 > ");
		int plantId = sc.nextInt();
		sc.nextLine();
		System.out.print("식물 이름을 입력해주세요 > ");
		String plantName = sc.nextLine();
		
		int result = pc.deletePlant(plantId, plantName);
		
		if(result > 0) {
			System.out.println("식물 정보삭제에 성공하셨습니다. ");
		} else {
	...
	private void findByPlant() {
		...
		System.out.println("=========== 식물 상세 조회 ============");
		System.out.print("검색할 식물 식별번호를 입력해주세요 > ");
		int plantId = sc.nextInt();
		sc.nextLine();
		
		Plant plant = pc.findByPlant(plantId);
		
		if(plant == null) {
			System.out.println("조회 결과가 존재하지 않습니다. ");
		} else {
			System.out.println("\n================================");
			System.out.print("식물 식별번호 : " + plant.getPlantId());
			System.out.print(" | 식물 이름 : " + plant.getPlantName());
			System.out.print(" | 식물 종류 : " + plant.getPlantType());
			System.out.print(" | 심은 날짜 : " + plant.getPlantedDate());
			System.out.print(" | 물주는 주기 : " + plant.getWateringCycle());
			System.out.println(" | 식물 성장 상태 : " + plant.getPlantStatus());
		}
	}
	
	private void findByKeyword() {
		
		System.out.println("========= 식물 이름 키워드로 검색 ============");
		System.out.print("검색할 식물이름 키워드를 입력해주세요 > ");
		String keyword = sc.nextLine();
		
		List<Plant> plant = pc.findByKeyword(keyword);

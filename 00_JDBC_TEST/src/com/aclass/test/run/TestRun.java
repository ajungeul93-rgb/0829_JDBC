...
=======
		 ...
		 * 
		 * 2. 접속하기 누름
>>>>>>> afbf8b7faf48381c4556258e16044b93c85751f7
		 * 
		 * 3. DBMS 선택
		 * 
		 * 4. ojdbc.jar --> 등록
		 * 
<<<<<<< HEAD
		 * 5. IP주소, PORT번호, 사용자계정, 비밀번호 갖고 연결
		 * 
		 * 6. 새 SQL편집기
		 * 
...
<<<<<<< HEAD
		...
		try {
			...
			// Driver등록은 프로그램 실행시 딱 1회만 하면됨
		    Class.forName("oracle.jdbc.driver.OracleDriver");
		    System.out.println("Driver등록 성공!");
		    
		    // 2) DB서버와의 연결(IP주소, PORT번호, 사용자이름, 비밀번호) /ip주소  / 포트번호
			...
			e.printStackTrace(); // 프로젝트에 OJDBC 추가했나요?
			System.out.println("무조건 56행이 문제");
		} catch(SQLException e) {
			e.printStackTrace();
			System.out.println("사용자 이름/ 비밀번호가 잘못됐습니다 -> 61,62행 문제");
			System.out.println("부적합한 Oracle URL이 지정되었습니다 -> 60행 문제");
			System.out.println("SQLSyntaxErrorException -> SQL문 확인하기/ 51행");
...
=======
		...
		try {
			...
			conn = DriverManager.getConnection("jdbc:oracle:thin:@115.90.212.20:10000", 
					                    "BJG12",
					                    "BJG121234");
			System.out.println("DB서버 접속 성공");
			...
			// UpdatedRows : N
			// UpdatedRows : N
			// DML(INSET, UPDATE, DELETE) => 처리된 행의 개수
			// executeUpdate(dml문);
			result = stmt.executeUpdate(sql);
			System.out.println("SQL문 실행 성공!");
			...
			e.printStackTrace(); // 프로젝트에 OJDBC추가 했나요?
			System.out.println("무조건 56행이 문제");
		} catch (SQLException e) {
			e.printStackTrace();
			System.out.println("사용자 이름/ 비밀번호가 잘못됐습니다. -> 61, 62행");
			System.out.println("부적합한 Oracle URL이 지정되었습니다 -> 60행");
			System.out.println("SQLSyntaxErrorException -> SQL문 확인하기");
			System.out.println("NullPointerException -> JBDC객체들 확인하기");
		} finally {
			try {
				  stmt.close();
				  conn.close();

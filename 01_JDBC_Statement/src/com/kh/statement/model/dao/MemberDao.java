...
public class MemberDao {
		 ...
		 * 
		 * 5) 결과 받기
		 *  > SELECT -> ResultSet(조회된 데이터들이 테이블 형태로 담김)객체로 받기
		 *  > DML    -> int(처리된 행 수)로 받기
		 * 
		 * 6)
		 *  > SELECT -> ResultSet에 담겨있는 데이터를 하나 하나씩 뽑아서 VO객체로 담기
	...
	public int save(Member member) {
		...
		try {
		     ...
		     conn = DriverManager.getConnection("jdbc:oracle:thin:@115.90.212.20:10000"
				                              , "BJG12"
				                              , "BJG121234");
		    // new Connection("jdbc:oracle:thin:...", "BJG12", "BJG121234")를 만든 느낌 conn에 객체 담기.
		    // 1. URL을 잘못 적었을 수 있음
